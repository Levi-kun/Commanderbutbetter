...
module.exports = {
	...
	run: (bot, message, args) => {
		...
		if(userinfoUser) {
			...
			if(userinfoUser.displayName === userinfoUser.user.username) {
				Nicknames = 'None'
			} else {
				Nicknames = message.member.displayName
			}
			let refuPic = userinfoUser.user.displayAvatarURL
			userEmbed.setColor('#495963')
			.setThumbnail(refuPic)
			.setAuthor(`${userinfoUser.user.username}:`)
			.addField('`Name`', userinfoUser.user.username, true)
			.addField('`Nickname`', userinfoUser.displayName, true)
			.addField('`Status`', userinfoUser.user.presence.status, true)
			.addField('`Highest Role`',  userinfoUser.highestRole)
			.addField('`Joined Discord`', userinfoUser.user.createdAt)
