const { Client, Collection, messageEmbed, MessageEmbed } = require('discord.js');
...
module.exports = async (bot, message) => {
	...
	function hook(channel, title, message, color, avatar) {
		...
		if (!message) {
			return console.log(
				'Aint no message high oof aint no message low oof.'
			);
		}
	...
	let savewords = JSON.parse(
		readFileSync('./json/savewords.json', 'utf8')
	);
...
*/
	...
	if (command.OwnerRequired === true) {
		if (!message.author.id === botconfig.botowner) {
			message.channel.send(`${message.author.username}, this is an owner only cmd.\n \*It's a dangeroud command!\*`);
			return;
		}
	}
	//
	...
	if (command) {
		...
		console.log(
			`${command.name} was used in ${message.channel.name} by ${message.author.tag}`
		);
