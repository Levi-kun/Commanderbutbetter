...
module.exports = async (bot, message) => {
	...
	function hook(channel, title, message, color, avatar) {
		...
		if (!channel)
			return console.log('Channel aint specified'); // thesse are the args or arguments.
	...
	if (message.content === 'dep is gay') {
		message.channel.send(
			`YOUR GAY DEP!!! HAHAHAHAHAH YOUR GAY!!!`
		);
	}
...
*/
	...
	if (command.OwnerRequired === true) {
		if (!message.author.id === botconfig.botowner) {
			let noOwnerValue = new RichEmbed()
